...
extension YTLiveStreaming {
  public func getUpcomingBroadcasts() async throws -> [LiveBroadcastStreamModel] {
    print("+\(#function)")
    return try await getBroadcastList(.upcoming)
  }
  
  public func getLiveNowBroadcasts() async throws -> [LiveBroadcastStreamModel] {
    print("+\(#function)")
    return try await getBroadcastList(.active)
  }
  
  public func getCompletedBroadcasts() async throws -> [LiveBroadcastStreamModel] {
    print("+\(#function)")
    return try await getBroadcastList(.completed)
  }
  
  public func getAllBroadcasts() async throws -> ([LiveBroadcastStreamModel]?, [LiveBroadcastStreamModel]?, [LiveBroadcastStreamModel]?) {
    let broadcastsResult = try await YTLiveRequest.listBroadcasts(.all)
    switch broadcastsResult {
    case .success(let broadcasts):
      let streams = fillList(broadcasts)
      ...
      return (streamsReady, streamsLive, streamsComplete)
  ...
  public func createBroadcast(_ title: String,
                              ...
                              startTime: Date,
                              privacy: String? = nil,
                              enableAutoStop: Bool? = nil) async throws -> LiveBroadcastStreamModel {
    ...
    let liveBroadcastResult = try await YTLiveRequest.createLiveBroadcast(title,
                                                                          liveStreamDescription,
                                                                          startDateTime: startTime,
                                                                          privacy: privacy,
                                                                          enableAutoStop: enableAutoStop)
    
    let liveBroadcast = try await liveBroadcastResult.get()
    
    let liveStreamResult = try await YTLiveRequest.createLiveStream(title,
                                                                    description: liveStreamDescription,
                                                                    streamName: liveStreamName)
    
    let liveStream = try await liveStreamResult.get()
    
    let bindResult = try await YTLiveRequest.bindLiveBroadcast(broadcastId: liveBroadcast.id,
                                                               liveStreamId: liveStream.id)
    
    let boundBroadcast = try await bindResult.get()
    
    return boundBroadcast
  }
  
  public func updateBroadcast(_ broadcast: LiveBroadcastStreamModel) async throws -> Bool {
    let result = try await YTLiveRequest.updateLiveBroadcast(broadcast)
  ...
  public func startBroadcast(_ broadcast: LiveBroadcastStreamModel,
    ...
    guard !broadcastId.isEmpty && !liveStreamId.isEmpty else {
      print("Please check broadcast.youtubeId. It has to contain broadcast Id and live stream Id")
      return (nil, nil, nil)
    }
    
    let liveBroadcastResult = try await YTLiveRequest.getLiveBroadcast(broadcastId: broadcastId)
    let liveBroadcast = try await liveBroadcastResult.get()
    
    let liveStreamResult = try await YTLiveRequest.getLiveStream(liveStreamId)
    let liveStream = try await liveStreamResult.get()
    ...
    let streamTitle = liveStream.snippet.title
    ...
    return (streamName, streamUrl, scheduledStartTime)
  }
  
  public func completeBroadcast(_ broadcast: LiveBroadcastStreamModel) async throws -> Bool {
    LiveLauncher.sharedInstance.stopBroadcast()
    let result = try await YTLiveRequest.transitionLiveBroadcast(broadcast.id, broadcastStatus: "complete")
    return result.isSuccess
  }
  
  public func deleteBroadcast(id: String) async throws -> Bool {
    let result = try await YTLiveRequest.deleteLiveBroadcast(broadcastId: id)
    return result.isSuccess
  }
  
  public func transitionBroadcast(_ broadcast: LiveBroadcastStreamModel,
                                  toStatus: String) async throws -> Bool {
    let result = try await YTLiveRequest.transitionLiveBroadcast(broadcast.id, broadcastStatus: toStatus)
    return result.isSuccess
  }
  
  public func getStatusBroadcast(_ broadcast: LiveBroadcastStreamModel,
                                 stream: LiveStreamModel) async throws -> (String?, String?, String?) {
    let broadcastResult = try await YTLiveRequest.getLiveBroadcast(broadcastId: broadcast.id)
    let broadcast = try await broadcastResult.get()
    
    let broadcastStatus = broadcast.status?.lifeCycleStatus ?? "complete"
    
    let liveStreamResult = try await YTLiveRequest.getLiveStream(stream.id)
    let liveStream = try await liveStreamResult.get()
    
    let streamStatus = liveStream.status.streamStatus
    let healthStatus = liveStream.status.healthStatus.status
    
    return (broadcastStatus, streamStatus, healthStatus)
  }
  
  public func transitionBroadcastToLiveState(liveBroadcast: LiveBroadcastStreamModel) async throws -> Bool {
    let transitionResult = try await transitionBroadcast(liveBroadcast, toStatus: "live")
    if transitionResult {
      print("Transition to the LIVE status was made successfully")
      return true
    } else {
      print("Failed transition to the LIVE status!")
      let testingResult = try await transitionBroadcast(liveBroadcast, toStatus: "testing")
      if testingResult {
...
extension YTLiveStreaming {
  fileprivate func getBroadcastList(_ status: YTLiveVideoState) async throws -> [LiveBroadcastStreamModel] {
    let broadcastsResult = try await YTLiveRequest.listBroadcasts(status)
    let broadcasts = try await broadcastsResult.get()
    return fillList(broadcasts)
  }
  
  fileprivate func fillList(_ broadcasts: LiveBroadcastListModel) -> [LiveBroadcastStreamModel] {
  ...
  fileprivate func deleteAllBroadcasts() async throws -> Bool {
    let broadcastsResult = try await YTLiveRequest.listBroadcasts(.all)
    let broadcasts = try await broadcastsResult.get()
    return await deleteBroadcasts(broadcasts.items, index: 0)
  }
  
  fileprivate func deleteBroadcasts(_ items: [LiveBroadcastStreamModel], index: Int) async -> Bool {
    if index < items.count {
      let item = items[index]
      let result = await deleteBroadcast(id: item.id)
      if result {
        print("Broadcast \"\(item.id)\" deleted!")
      }
      return await deleteBroadcasts(items, index: index + 1)
    } else {
      return true
    }
  }
  
  fileprivate func deleteBroadcast(id: String) async -> Bool {
    let result = await YTLiveRequest.deleteLiveBroadcast(broadcastId: id)
    return result.isSuccess
  }
}
...
extension YTLiveStreaming {
  func testUpdateLiveStream() async {
    let liveStreamId = "0"
    let title = "Live Stream"
    let format = "1080p"    // 1080p 1440p 240p 360p 480p 720p
    let ingestionType = "rtmp" // dash rtmp
    do {
      try await YTLiveRequest.updateLiveStream(liveStreamId,
                                               title: title,
                                               format: format,
